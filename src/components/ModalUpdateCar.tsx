...
export const ModalUpdateCar = ({ id, onSubmit }: { id: number, onSubmit: () => void }) => {
  ...
  const handleFormSubmit = (data: FormData) => {
    ...
    updateCar(id, formData, `${userToken}`, onSubmit);
  }
  ...
  return (
    ...
    <Dialog.Trigger asChild>
      <button className="px-3 py-2 mt-3 text-sm font-bold text-white duration-150 ease-in-out bg-blue-500 rounded hover:bg-blue-700 hover:scale-95">
        Editar carro
      </button>
    </Dialog.Trigger>
    <Dialog.Portal>
      ...
      <Dialog.Content className="data-[state=open]:animate-contentShow fixed top-[50%] left-[50%] max-h-[85vh] w-[90vw] max-w-[450px] translate-x-[-50%] translate-y-[-50%] rounded-[6px] bg-white p-[25px] shadow-[hsl(206_22%_7%_/_35%)_0px_10px_38px_-10px,_hsl(206_22%_7%_/_20%)_0px_10px_20px_-15px] focus:outline-none">
        ...
        <Dialog.Description className="text-zinc-700 mt-[10px] mb-5 text-[15px] leading-normal">
          Digite as informações do carro a ser atualizado no catálogo.
        </Dialog.Description>
